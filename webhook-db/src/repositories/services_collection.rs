use {
    crate::{
        ...
        schema::{service_collection, services},
    },
    chrono::Local,
    diesel::prelude::*,
...
impl ServicesCollection {
    ...
    pub async fn get(
        ...
        let ret = service_collection::table
            .filter(service_collection::created_by.eq(created_by))
            .filter(service_collection::id.eq(id.unwrap_or_default()))
            .or_filter(service_collection::client_id.eq(client_id.unwrap_or_default()))
            .select(ServiceCollection::as_select())
            .first(&mut conn)
            .await?;
    ...
    pub async fn check_exist_service_collection(
        ...
        let ret = service_collection::table
            .filter(service_collection::client_id.eq(client_id.unwrap_or_default()))
            .or_filter(service_collection::id.eq(id.unwrap_or_default()))
            .select(ServiceCollection::as_select())
            .first(&mut conn)
            .await?;
    ...
    pub async fn register_service_collection(
        ...
        if !service_collection.client_id.is_empty() && service_collection.address == address {
            return Err(ServiceError {
    ...
    pub async fn un_register_service_collection(
        ...
        let service_collection = self
            ...
            .await
            .unwrap_or(ServiceCollection::default());
        if service_collection.client_id.is_empty() || service_collection.address.is_empty() {
            return Err(ServiceError {
    ...
    pub async fn update_start_block_number(
        ...
        let mut conn = self.db.get_connection().await;
        let ret =
            diesel::update(service_collection::table.filter(service_collection::id.eq(id.clone())))
                .set((
