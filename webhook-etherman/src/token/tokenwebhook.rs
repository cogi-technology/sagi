use {
    ...
    webhook_db::{
        ...
        models::StatusEvent,
        repositories::{
            services_token::ServicesToken, services_token_webhook::ServicesTokenWebhook,
...
pub struct TokenWebhook {
    ...
    service_token_webhook_db: Arc<ServicesTokenWebhook>,
...
impl TokenWebhook {
    ...
    pub fn get_service_webhook_db(&self) -> Arc<ServicesTokenWebhook> {
        Arc::clone(&self.service_token_webhook_db)
    }
...
impl TokenWebhook {
    pub async fn init(db: Arc<Database>, private_key_path: String) -> Result<Self> {
        ...
        let service_token_webhook_db = Arc::new(ServicesTokenWebhook::new(Arc::clone(&db)));

        Ok(Self {
            event_db,
            service_token_db,
            service_token_webhook_db,
            private_key_path,
        })
    }
    ...
    pub async fn heartbeat_event(&self) -> Result<()> {
        ...
        loop {
            // get all contract
            let m = self.service_token_webhook_db.get_all().await?;
            for s in m {
                let lst_events = self
                    .event_db
