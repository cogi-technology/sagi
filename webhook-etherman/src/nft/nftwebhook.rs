use {
    ...
    webhook_db::{
        ...
        repositories::{
            ...
            services_collection_webhook::ServicesCollectionWebhook,
        },
    },
};

pub struct NFTWebhook {
    event_db: Arc<NftEvents>,
    service_collection_db: Arc<ServicesCollection>,
    service_webhook_db: Arc<ServicesCollectionWebhook>,
...
impl NFTWebhook {
    ...
    pub fn get_service_webhook_db(&self) -> Arc<ServicesCollectionWebhook> {
        Arc::clone(&self.service_webhook_db)
    }
...
impl NFTWebhook {
    pub async fn init(db: Arc<Database>, private_key_path: String) -> Result<Self> {
        ...
        let service_webhook_db = Arc::new(ServicesCollectionWebhook::new(Arc::clone(&db)));
    ...
    pub async fn heartbeat_event(&self) -> Result<()> {
        ...
        loop {
            // get all contract
            let m = self.service_webhook_db.get_all().await?;
            for s in m {
                let lst_events = self
                    .event_db
                ...
                for e in lst_events {
                    ...
                    if e.collection == collection_rej
                        && e.token_id.is_some()
                        && Some(e.token_id) == Some(token_rej)
                    {
                        continue;
                    }
                    // send endpoint
